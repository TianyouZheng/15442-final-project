...
def NewPress(compression_ratio: float = 0.5):
    press_classes = [TOVAPress, ExpectedAttentionPress]

    # total ratio = ratio 1 * ratio 2 * ratio 3 ...
    each_ratio = math.pow(compression_ratio, 1 / len(press_classes))

    press_objs = [press(compression_ratio=each_ratio) for press in press_classes]

    return ComposedPress(press_objs)
    ...
    for quantization in quantizations:
        for compression_ratio in compression_ratios:
            for press in presses:
                ...
                if hasattr(press_obj, "head_compression_ratio") and not isinstance(press_obj, ComposedPress):
                    press_obj.head_compression_ratio = compression_ratio
                elif hasattr(press_obj, "compression_ratio") and not isinstance(press_obj, ComposedPress):
                    press_obj.compression_ratio = compression_ratio

                max_new_tokens = 128 if compression_ratio > 0.5 else 1024
