...
from transformers import pipeline, QuantizedCacheConfig, HQQQuantizedCache
...
def make_llm(
    ...
    kwargs = {
        ...
        "question": """Your task is to review and critique the solution paragraph by paragraph. Once you identify an error in a paragraph, return the index of the paragraph where the earliest error occurs. Otherwise, return the index of -1 (which typically denotes "not found"). Please only write your final answer (i.e., the index) in \\boxed{{}}.""",
    ...
    if quantization is not None:
        kwargs["cache"] = HQQQuantizedCache(QuantizedCacheConfig(nbits=quantization, device=device))
    ...
    def generate(prompts):
        ...
        for prompt in tqdm_obj:
            ...
            print(result)
            # with open("./quantization_outputs/hqq.txt", "a") as f:
            #     f.write(result + "\n")
        return results
